...
`Предполагаемый_Пиковый_Трафик_загрузки_трека = 4.8 * 10^6 чел * 0.7 * 22 трека * 5.86 Мб * 8 бит / (12 ч * 60 мин * 60 с) ~= 73.34 Гбит/с`
...
Для хранения основных данных сервиса будем использовать PostgreSQL. Данные о треках, артистах и альбомах не требуют постоянных изменений. Для хранения этой информации, а так же плейлистов пользователей можно воспользоваться одной БД с 2 репликами по приницпу master-slave-slave. Рассчитаем объем памяти:
...
Для данных пользователя (28.59 млн записей):
 ...
 ID | Название(varchar 64) | Продолжительность(integer 2 байта) | Итого
 -|-|-|-
 4 байта| 256 байт | 2 байта | 302 байта

 Для данных альбомов (Предполагаем, что средний альбо будет содержать 10 треков = 3.5 млн альбомов):
...
`Количество_данных = 28.59 * 10^6 * 1324 байт + 35 * 10^6 * 302 байт + 3.5 * 10^6 * 304 байт + 7 * 10^5 * 300 байт + 2.9 * 10^9 * 16 байт + (7 * 10^5 исполнителей * 50 треков * 16 байт + 7 * 10^5 исполнителей * 5 альбомов + 3.5 * 10^6 альбомов * 10 треков) = 96695660000 байт ~= 90 Гб`
...
Пиковый трафик CDN с учетом запаса в 30% должен составлять `~96 Гбит/с`. При этом объем хранимых данных должен составить минимум `195 Тб` для текущей медиатеки + 30% для запаса на добавления новых медиаданных, т.е. `255 Тб`.
Максимальная скорость раздачи, которая мы можем обеспечить это `40 Гбит/с` при оснащении сервера 4 CPU. Для шардирования контента разделим медиатеку на четыре сервера с размером диска в `128 Гб`. 4 сервера данных снабдим 2 репликами каждый и получим 12 серверов для хранения контента. Каждый кластер из 12 серверов снабдим балансировщиком nginx. Идеальным будет распределить  наши сервера по 3 зонам. Тогда необходимо 36 серверов + 3x2 балансировщика.
...
### Итоговая конфигурация
 ...
 Tarantool сесии | 16 | 128 Гб | 10Gx1 | 128 Гб SSD | 2
 PostgreSQL | 16 | 128 Гб | 10Gx1 | 1 Тб SSD | 3
 Tarantool история | 16 | 256 Гб | 10Gx1 | 128 Гб SSD | 2
 Clickhouse история | 16 | 128 Гб | 10Gx1 | 4 Тб SSD | 2
 Бекенд | 32 | 128 Гб | 10Gx1 | 128 Гб SSD | 4
 Фронтенд | 16 | 64 Гб | 10Gx1 | 128 Гб SSD | 2
 Балансировка | 32 | 64 Гб | 10Gx1 | 128 Гб SSD | 2 фронт + 2 бек + 6 cdn 
 CDN | 32 | 256 Гб | 10Gx4 | 128 Гб SSD | 36
...
Для серверов CDN я выбрал расположения в Москве и Санкт-Петербурге. В Москве необходимо выделить дополнительный кластер для работы с Азиатской частью России.

## Схема балансировки нагрузки

Для выбора ближайшего CDN сервера используем Geo-based DNS технологию, с помощью которой контент будет отдаваться из ближайшего к пользователю ДЦ. Внутри каждого кластера CDN используется свой nginx Layer 7 Load Balancing.
Данная технология позволяет равномерно распределить нагрузку, позволяет кешировать данные, передавать их в сжатом виде. Nginx сервер обладает встроенными средствами для SSL терминации.
