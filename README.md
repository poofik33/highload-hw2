...
Для хранения сессий пользователя будем использовать еще один инстанс Redis с репликой, так как для каждого действия пользователя необходима проверка на авторизацию (для блокирования функций или изменения, например, прогигрывания рекламы перед треком).
...
Пиковый RPS на бекенд составляет `3300 rps`, что с учетом запаса в 30% составляет `4300 rps`.
...
В качестве облачного провайдера для разворачивания сервера я решил выбрать MCS, который обладает всем необходимым функционалом для развертывания нашего сервиса, плюс сервера расположены в России. В качестве распределения по дата центрам предполагается выбрать, исходя из данных о 70% населения России, живущего в Европейской части, Москву, Санкт-Петербург и Новосибирск. Основная часть серверов будет расположена в первых двух городах из списка.

## Схема балансировки нагрузки

Для выбора ближайшего CDN сервера используем Geo-based DNS технологию, с помощью которой контент будет отдаваться из ближайшего к пользователю ДЦ.

Для балансировки нагрузки на API используем Layer 7 Load Balancing на основе nginx. Данная технология позволяет равномерно распределить нагрузку, позволяет кешировать данные, передавать их в сжатом виде. Nginx сервер обладает встроенными средствами для SSL терминации.

## Обеспечение отказоустойчивости

Отказоустойчивость в проекте обеспечивает за счет репликации серверов БД, а также входного nginx сервера. Также формирование серверов API производится с учетом запаса в 30% от пиковой нагрузки, что позволяет держать в запасе несколько серверов, готовых принять нагрузку в случае отказа нескольких серверов.